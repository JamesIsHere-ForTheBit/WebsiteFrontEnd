...
// REMOVE BEFORE PRODUCTION — demo admin login only when CONFIG.api.baseUrl is empty; real credentials must live server-side only.
const DEMO_ADMIN_EMAIL = 'itadmin@adams6.local';
const DEMO_ADMIN_PASSWORD = 'ITAdmin2026!';

function generateDemoOrderNumber() {
    return 'ORD' + Math.floor(Math.random() * 1000000);
}

/** If API returns JSON with a usable order id, return it; otherwise null (caller falls back to demo generator). */
function extractOrderNumberFromApiData(data) {
    if (!data || typeof data !== 'object') return null;
    const raw = data.orderNumber ?? data.order_number ?? data.orderId ?? data.id;
...
let isGuest = false;
/** Set when login modal was opened because user clicked Proceed to Checkout without an account or guest session. */
            ...
            cartItem.innerHTML = `
                ...
                <div class="cart-item-details">
                    ...
                    <div class="cart-item-quantity">
                        ...
                        <button class="quantity-btn" onclick="updateQuantity(${item.id}, 1)">+</button>
                        <button type="button" class="quantity-btn quantity-btn-remove" onclick="removeFromCart(${item.id})" title="Remove" aria-label="Remove"><i class="fas fa-trash" aria-hidden="true"></i></button>
                    </div>
                </div>
            `;
...
function continueAsGuest() {
    isGuest = true;
    closeLoginModal();
    const resumeCheckout = pendingCheckout;
    pendingCheckout = false;
    if (resumeCheckout) {
        beginCheckout();
    } else {
...
// ORDER HISTORY
// ============================================
function saveOrderToHistory(subtotal, servicesTotal, grandTotal, orderNumber) {
    const order = {
        orderNumber: orderNumber || generateDemoOrderNumber(),
...
// CHECKOUT
// ============================================
function validateCheckoutPreconditions() {
    if (cart.length === 0) {
...
function beginCheckout() {
    if (!validateCheckoutPreconditions()) return;
    closeCart();
...
async function placeOrder(event) {
    ...
    if (!validateCheckoutPreconditions()) {
        return;
    }
    ...
    const shipping = {
        ...
        cityStateZip: document.getElementById('shippingCityStateZip').value.trim(),
        email: document.getElementById('shippingEmail').value.trim()
    };

    const payment = {
        cardNumber: document.getElementById('paymentCard').value,
    ...
    if (CONFIG.api.baseUrl) {
        ...
        const apiRes = await API.post(CONFIG.api.endpoints.orders, payload);
        resolvedOrderNumber = extractOrderNumberFromApiData(apiRes);
        if (!apiRes) {
            console.warn('Order API request failed — using local order number on receipt.');
            alert('Could not reach the order server. Showing a local receipt.');
        } else if (!resolvedOrderNumber) {
            console.warn('Server response did not include an order number — using a local id.');
        }
    }

    if (!resolvedOrderNumber) {
        resolvedOrderNumber = generateDemoOrderNumber();
    }

    if (isLoggedIn) {
        saveOrderToHistory(subtotal, servicesTotal, grandTotal, resolvedOrderNumber);
        showOrderHistory();
    }

    showReceipt(subtotal, servicesTotal, grandTotal, resolvedOrderNumber);
...
function showReceipt(subtotal, servicesTotal, grandTotal, orderNumberForDisplay) {
    const receiptContent = document.getElementById('receiptContent');
    const date = new Date();
    const orderNumber =
        orderNumberForDisplay && String(orderNumberForDisplay).trim().length > 0
            ? String(orderNumberForDisplay).trim()
...
function recordAdminLoginFailure(email, now) {
...
function mapApiUserToSessionUser(u, emailFallback) {
    ...
    return {
        id: u.id != null ? u.id : Date.now(),
        email: u.email || emailFallback,
        name: u.name || 'Admin',
        isAdmin: u.isAdmin !== undefined ? u.isAdmin : true,
...
async function handleAdminLogin(event) {
    ...
    const mfaVisible = mfaGroup.style.display === 'block';
    const now = Date.now();
    ...
    if (CONFIG.api.baseUrl) {
        if (!mfaVisible) {
            const data = await API.post(CONFIG.api.endpoints.adminLogin, { email, password });
            if (!data) {
                alert('Invalid credentials');
                recordAdminLoginFailure(email, now);
                return;
            }
            if (data.requiresMfa || data.requireMFA) {
                mfaGroup.style.display = 'block';
                alert('Enter your MFA code.');
                return;
            }
            const token = data.token || data.access_token || null;
            const userPayload = data.user != null ? data.user : data;
            const loginOk = token || (data.user && typeof data.user === 'object');
            if (loginOk) {
                delete loginAttempts[email];
                const user = mapApiUserToSessionUser(userPayload, email);
                user.isAdmin = true;
                const session = SessionManager.setSession(user, token || ('api-token-' + Date.now()));
        ...
        if (!mfa || String(mfa).trim().length === 0) {
            alert('Enter your MFA code.');
            return;
        }

        const verifyData = await API.post(CONFIG.api.endpoints.verifyMFA, { email, code: mfa });
        if (!verifyData) {
            alert('Invalid MFA code');
            recordAdminLoginFailure(email, now);
            return;
        }
        const token = verifyData.token || verifyData.access_token || null;
        const userPayload = verifyData.user || verifyData;
        delete loginAttempts[email];
        const user = mapApiUserToSessionUser(userPayload, email);
        user.isAdmin = true;
        const session = SessionManager.setSession(user, token || ('api-token-' + Date.now()));
        AuditLogger.log('admin_login', 'Admin logged in after MFA');
        completeAdminLogin(session);
        return;
    }
...
window.onclick = function(event) {
    const modals = ['ageModal', 'loginModal', 'cartModal', 'checkoutModal', 'receiptModal', 'adminLoginModal', 'userDetailsModal'];
    modals.forEach(id => {
